    ...
    while not rospy.is_shutdown():
        ...
        if last_msg is not None:
            ...
            if len(r_axes) >= 6:
                ...
                axes[4] = float(r_axes[4]) # L2
                axes[5] = float(r_axes[5]) # R2
            ...
            if len(r_btns) >= 11:
                ...
                buttons[6] = int(r_btns[4])  # Pair
                buttons[7] = int(r_btns[6])  # Menu
                # buttons[8] = int(r_btns[15])  # Guide 鍵
                # buttons[9] = int(r_btns[7])  # 左搖桿下壓 (L3)
                # buttons[10] = int(r_btns[8]) # 右搖桿下壓 (R3)
                buttons[11] = int(r_btns[11]) # 十字鍵 上
                buttons[12] = int(r_btns[12]) # 十字鍵 下
                buttons[13] = int(r_btns[13]) # 十字鍵 左
                buttons[14] = int(r_btns[14]) # 十字鍵 右
