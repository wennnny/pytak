...
def posearray_callback(msg: PoseArray):
    try:
        for idx, pose in enumerate(msg.poses):
            ...
            rospy.loginfo(f"[UDP] Sent Start[{idx}] -> x: {x:.6f}, y: {y:.6f}, z: {z:.6f} [len={len(packet)}]")
...
def main():
    ...
    rospy.init_node('start_udp_sender', anonymous=True)
    UDP_TARGET_IP = rospy.get_param("~target_ip", "10.0.0.80")
    UDP_TARGET_PORT = rospy.get_param("~udp_port", 49153)

    rospy.Subscriber("/start_point/gps/pose_array", PoseArray, posearray_callback)
    rospy.loginfo(f"Listening to /start_point/gps/pose_array and sending each Pose via UDP to {UDP_TARGET_IP}:{UDP_TARGET_PORT}...")
    rospy.spin()
