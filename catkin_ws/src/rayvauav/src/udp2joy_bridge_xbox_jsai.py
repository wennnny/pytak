    ...
    while not rospy.is_shutdown():
        ...
        if last_msg is not None:
            ...
            if len(r_axes) >= 6:
                ...
                axes[4] = float(r_axes[4]) # LT
                axes[5] = float(r_axes[5]) # RT
            
            # 處理十字鍵 (Hats) 映射到 Axes[6] 與 Axes[7]
            if isinstance(r_hats, list) and len(r_hats) > 0 and len(r_hats[0]) == 2:
                axes[6] = float(r_hats[0][0]) # 十字鍵 左右
                axes[7] = float(r_hats[0][1]) # 十字鍵 上下
            ...
            if len(r_btns) >= 11:
                ...
                buttons[4] = int(r_btns[4])  # LB
                buttons[5] = int(r_btns[5])  # RB
                buttons[6] = int(r_btns[6])  # Map
                buttons[7] = int(r_btns[7])  # Menu
                # buttons[8] = int(r_btns[10])  # Guide 鍵
                # buttons[9] = int(r_btns[8])  # 左搖桿下壓 (L3)
                # buttons[10] = int(r_btns[9]) # 右搖桿下壓 (R3)
